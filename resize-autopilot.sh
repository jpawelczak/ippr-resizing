...
kubectl patch pod resize-demo-g --subresource resize --patch \
  '{"spec":{"containers":[{"name":"demo-g", "resources":{"requests":{"memory":"1.5G"},"limits":{"memory":"1.5G"}}}]}}' \
  && echo "scale-down Mem req and limits to 1.5G"
...
kubectl patch pod resize-demo-g --subresource resize --patch \
  '{"spec":{"containers":[{"name":"demo-g", "resources":{"requests":{"cpu":"0.250","memory":"0.600G"},"limits":{"cpu":"0.250","memory":"0.600G"}}}]}}' \
  && echo "scale-down to 0.250 CPU and to 0.600 Mem"
